...
const csvEmbedding = async (inputFile) => {
  try {
    ...
    logger.info(`Processing CSV with headers: category="${categoryHeader}", comment="${commentHeader}"`);
    ...
    logger.info(`Successfully wrote embeddings to ${standardPath} (${stats.size} bytes)`);
